document.addEventListener('DOMContentLoaded', () => {
    ...
    function displayCertificate(canvas) {
        ...
        // Create mobile-only open in browser button
        createMobileOpenInBrowserButton(canvas);
        
        // Automatically scroll to the certificate preview
        certificatePreview.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }

    // Function to create a button for mobile users to open in external browser
    function createMobileOpenInBrowserButton(canvas) {
        // First remove any existing open in browser button
        const existingBtn = document.getElementById('openInBrowserBtn');
        if (existingBtn) {
            existingBtn.remove();
        }
